...
[`AutoDataSavingService`](https://github.com/Tanmayjihuma/RobloxSkill/blob/main/scripts/AutoDataSavingService.lua) periodically scans the player and also runs when the player leaves. 
It extracts: 
- Live Attributes 
...
|`UnreliableRemoteEvents`|Any direction|No|High-frequency updates where dropped packets are fine|
...
- For inventry whose size is not fix its hard to make perfect ui for it here is solution -> To make a perfect responsive inventory, set your ScrollingFrame's AutomaticCanvasSize to Y with CanvasSize Y at {0,0}, add a UIGridLayout using scale in X of  CellSize and Y of CellSize must be {1,0} and Scale for X and Y CellPadding, and insert a UIAspectRatioConstraint directly inside the layout with DominantAxis set to Width.
...
_AI Pitfall: Always use_ _`Animator:LoadAnimation()` . Calling this_ _on the Humanoid is deprecated._ 
...
|Anchor static parts when needed or necessary|Medium|Reduce physics budget|
|Limit Point Lights when needed or necessary|High|Any scene with many lights|
